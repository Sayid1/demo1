<template>
	<view class="m-tabbar-box" :style="tabbarBoxStyle" v-if="isShowTabBar">
		<view
			class="m-tabbar__fill"
			v-if="fill || native"
			:class="{ 'm-tabbar__safe': safeBottom || native }"
			:style="tabbarFillStyle"
		/>
		<view
			id="m-tabbar"
			class="m-tabbar"
			:class="{ fixed: fixed || native, 'm-tabbar__safe': safeBottom || native }"
			:style="tabbarStyle"
		>
...
export default {
	...
	computed: {
		tabbarConfig() {
			const { isReload, native, initTabbarConfig: tabbar } = this
			if (isReload) {
			}
			// 如果是native模式，直接读取执行
			if (native) {
		...
		borderStyle() {
			const { isReload, borderStyle } = this.tabbarConfig
			if (isReload) {
			}
			return borderStyle
		},
		tabbarBoxStyle() {
			const { isReload, zIndex } = this
			if (isReload) {
			}
			return obj2strStyle({
				'z-index': zIndex
		...
		tabbarFillStyle() {
			const { isReload, tabbarHeight } = this
			if (isReload) {
			}
			return obj2strStyle({
		...
		tabbarItemStyle() {
			const {
				isReload,
				currentIndex,
				tabbarConfig: { color, selectedColor }
			} = this
			if (isReload) {
			}
			return obj2strStyle({
	...
	methods: {
		_initTabbar() {
			const { current, fill, native, tabbarList } = this
		...
		setTabBarItem(obj) {
			const { index, text, pagePath: newPagePath, iconPath, selectedIconPath } = obj
			const { pagePath: oldPagePath } = this.tabbarConfig.list[index]
			if (this._checkMaxIndex(index)) {
