<template>
	<view :class="{ 'u-sticky': cssSticky }" :id="elId" :style="[style]">
		<view :style="[stickyContent, contentStyle]" class="u-sticky__content">
			<slot />
		</view>
	</view>
</template>
...
export default {
	...
	computed: {
		...
		stickyContent() {
			...
			let { isFull } = this
			let sys = uni.$u.sys()
			if (!this.cssSticky && !this.disabled) {
				style.position = this.fixed ? 'fixed' : 'static'
				style.top = (this.topPos === 'auto' ? this.stickyTop : this.topPos) + 'px'
				style.left = (this.leftPos === 'auto' ? this.left : this.leftPos) + 'px'
				style.width = this.width == 'auto' ? 'auto' : this.width + 'px'
				style.zIndex = this.uZindex

				if (isFull && this.fixed) {
					style.width = '100%'
					style.left = 0
					// 初始化不在首屏 比如swiper上
					style.paddingLeft = (this.left % sys.screenWidth) + 'px'
				}
			}
			return style
		},
	...
	methods: {
		...
		getRect() {
			return this.$uGetRect('#' + this.elId).then(res => {
		...
		observeContent() {
			...
			vm = this
			// #endif
			const contentObserver = vm.createIntersectionObserver({
				// 检测的区间范围
