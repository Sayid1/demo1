	...
	>
		<template v-if="src">
			...
			>
				...
				<u-icon v-else :name="loadingIcon" :width="width2" :height="height2"></u-icon>
			</view>
			<view
			...
			>
				...
				<u-icon v-else :name="errorIcon" :width="width2" :height="height2"></u-icon>
			</view>
		</template>
	</view>
</template>
...
export default {
	...
	computed: {
		// 只支持网络资源，只对微信小程序有效
		webp() {
			return this.src.indexOf('.webp') !== -1
		},
		width2() {
			let { width } = this
		...
		wrapStyle() {
			...
			style.width = this.$u.addUnit(this.width2)
			style.height = this.$u.addUnit(this.height2)
