	...
	<view class="s-pull-scroll" :class="customClass" :style="{ height }">
		...
		>
			<view>
				...
				<view
					:class="{
						'empty-contariner-full': isFullEmpty
					}"
					v-if="isEmpty && !hideEmpty"
				>
					<slot name="empty"></slot>
				</view>

				<view
					class="s-pull-up-wrap"
					:style="[footerStyle]"
					v-if="
						enablePullUp &&
							!isEmpty &&
							(isUpLoading ||
								(isUpError && showUpError) ||
								(isUpFinish && showUpFinish))
					"
				>
...
export default {
	...
	mounted() {
		this.selectorQuery('.s-pull-scroll').then(ret => {
			this.scrollViewHeight = ret.height
		})
	},
	methods: {
		...
		touchmove(e) {
			...
			if (
				moveY > 0 &&
				(scrollTop <= 0 ||
					(scrollTop <= this.numDownStartTop && scrollTop === this.startTop))
			) {
				// 可下拉的条件
				if (
					this.pullDown &&
					...
					// 设置旋转角度
					this.downRotate =
						'rotate(' + 360 * (this.downHight / this.numDownOffset) + 'deg)'
				}
			}
			// 记录本次移动的点
