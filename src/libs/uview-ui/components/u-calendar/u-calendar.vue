...
export default {
	...
	computed: {
		...
		subtitle() {
			...
			if (this.months.length) {
				return {
					year: this.months[this.monthIndex].year,
					month: this.months[this.monthIndex].month
				}
				// `${this.months[this.monthIndex].year}年${
	...
	methods: {
		...
		setMonth() {
			...
			this.updateMonthTop(this.topArr)
		},
		// 滚动到默认设置的月份
		...
		onScroll(event) {
			...
			for (let i = 0; i < this.months.length; i++) {
				if (scrollTop + 70 >= (this.months[i].top || this.listHeight)) {
					this.monthIndex = i
				}
			}
		},
