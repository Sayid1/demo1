			...
			<view class="u-calendar-month__days">
				...
				>
					...
					>
						...
						>
							<!-- 天数样式 -->
							<view class="custom-style" :style="[getIconStyle(item1)]">
								{{ item1.day }}
							</view>
						</view>
						<view
							...
							:class="[
								...
								(getBottomInfo(index, index1, item1).isRecord == 2 ||
									(getBottomInfo(index, index1, item1).isTOday && isTutelage)) &&
									'bulu',
								getBottomInfo(index, index1, item1).title == '已订阅' && 'subscribe'
							]"
							:style="[textStyle(item1)]"
						>
							<template v-if="getBottomInfo(index, index1, item1).isRecord != 2">
								<template v-if="!isTutelage">
									<view
										class="box-wrap"
										v-if="
											getBottomInfo(index, index1, item1).ons ||
												getBottomInfo(index, index1, item1).measureOns
										"
									>
										<image
											class="icon"
											:src="$ui.getCdnImage('calendar/ons.png')"
											mode="widthFix"
										></image>
										<view>
											{{
								...
								<template v-else>
									<view
										class="box-wrap"
										v-if="
											!getBottomInfo(index, index1, item1).isTOday &&
												(getBottomInfo(index, index1, item1).ons ||
													getBottomInfo(index, index1, item1).measureOns)
										"
									>
								...
								<view
									class="box-wrap"
									v-if="
										getBottomInfo(index, index1, item1).weight &&
											!getBottomInfo(index, index1, item1).isTOday
									"
								>
									<image
										class="icon"
										:src="$ui.getCdnImage('calendar/weight.png')"
										mode="widthFix"
									></image>
									<view>
										{{ getBottomInfo(index, index1, item1).weight }}
								...
								<view
									class="box-wrap"
									v-if="getBottomInfo(index, index1, item1).title === '提醒'"
								>
									<image
										class="icon"
										:src="$ui.getCdnImage('calendar/bell.png')"
										mode="widthFix"
									></image>
								</view>
							</template>
							...
							>
								{{
									getBottomInfo(index, index1, item1).isTOday && isTutelage
										? '未记录'
										: getBottomInfo(index, index1, item1).title || ''
								}}
							</text>
							<u-button
								v-if="
							...
							<u-button
								v-if="
									(getBottomInfo(index, index1, item1).isRecord == 2 &&
										...
										(getBottomInfo(index, index1, item1).isTOday && isTutelage)
								"
								:customStyle="bellStyle"
								text="去提醒"
							></u-button>
						</view>
						<text
							v-if="getBottomInfo(index, index1, item1).isTOday"
							class="today_style"
						>
							今
						</text>
...
export default {
	...
	computed: {
		...
		daySelectStyle() {
			return (index1, index2, item) => {
				...
				if (item.disabled) {
					...
					if (item.bottomInfo && item.bottomInfo.isRecord === 2) {
						style.backgroundColor = '#FFEBEB'
						style.border = '2rpx solid #ECA8A8'
					} else {
						style.backgroundColor = '#F2F8FF'
						style.border = '2rpx solid #C7D4E4'
					}
				}
				if (date === this.selected[0] && !item.bottomInfo.measureOns) {
					style.backgroundColor = '#F1E5D6'
					style.border = '2rpx solid #C99842'
				}
				return style
			}
		},
		...
		getIconStyle() {
			return item => {
				const date = dayjs(item.date).format('YYYY-MM-DD')
				if (item.disabled && this.selected[0] != date) {
					return {
						'background-image': `url(${this.$ui.getCdnImage('calendar/gray.png')})`,
						'background-size': '100% 100%',
						'background-position': 'center center',
						color: '#666666'
					}
				} else if (item.bottomInfo.title) {
					if (this.selected[0] == date) {
						return {
							'background-image': `url(${this.$ui.getCdnImage(
								'calendar/yellow.png'
							)})`,
							'background-size': '100% 100%',
							'background-position': 'center center',
						...
						return {
							'background-image': `url(${this.$ui.getCdnImage('calendar/red.png')})`,
							'background-size': '100% 100%',
							'background-position': 'center center',
							color: '#fff'
						}
					...
					return {
						'background-image': `url(${this.$ui.getCdnImage('calendar/icon1.png')})`,
						'background-size': '100% 100%',
						'background-position': 'center center'
					}
				}
	...
	methods: {
		...
		getMonthRect() {
			...
			Promise.all(promiseAllArr).then(sizes => {
				...
				console.log(topArr,222222,';213123');
